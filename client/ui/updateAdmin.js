...
export function updateUI() { // Renamed function locally
    ...
    if (uiElements.leaderboardListUI) { const rows = []; const playersToDisplay = Object.values(allConnections).filter(p => !p.isSpectator); playersToDisplay.sort((a, b) => { if (phase === 'lobby' || phase === 'countdown') { return (a?.playerName || '').localeCompare(b?.playerName || ''); } else { return (b?.seedCount ?? 0) - (a?.seedCount ?? 0); } }); playersToDisplay.forEach(player => { const isMe = player.id === gameState.myId; let status = ''; if (player.isSpectator) status = ' (Spectating)'; else if (phase === 'playing' || phase === 'ended') status = player.isAlive ? '' : ' (Dead)'; else if (phase === 'lobby' || phase === 'countdown') status = player.hasChosenSpawn ? ' (Placed)' : ''; const name = player.playerName || `Player ${player.id.substring(0,4)}`; const seeds = (phase === 'playing' || phase === 'ended') ? `: ${player.seedCount} Seeds` : ''; rows.push({ text: `${name}${status}${seeds}`, bold: isMe }); }); if(rows.length === 0 && phase !== 'loading') { if (Object.keys(allConnections).length > 0 && playerCount === 0) rows.push({ text: 'Only spectators connected...' }); else if (Object.keys(allConnections).length === 0) rows.push({ text: 'Waiting for server...' }); } renderListItems(uiElements.leaderboardListUI, rows); }
